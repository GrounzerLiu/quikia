...
use crate::property::{ColorProperty, FloatProperty, Gettable, Observable, Observer, SharedProperty, TextProperty};
...
impl TextBlock {
    pub fn new() -> Self {
        //let text = TextProperty::from_value(StyledText::from_str(""));
                                ...
                                match pointer_action {
                                    PointerAction::Down { x, y, pointer_type } => {
                                        if item.get_focusable().get() && item.get_focusable_when_clicked().get() {
                                            item.get_app().request_focus(item.get_item_path());
                ...
                .set_on_ime_input(
                    {
                        ...
                        move |item, ime_action| {
                            ...
                            item.get_app().request_re_layout();
                        }
